...
class TodoService {
  ...
  loadTodoList() {
    ...
    TodoEvent.getInstance().addEventModifyTodoClick(); // 내가 쓰지 못한 코드
  }
}

// 데이터를 추가할때 가장 효율적인건 제일 끝에 추가시키는것(반복 안돌려도 됨), but 쓰면 안됨
// 추가하는순간에 DB에서 처음부터 끝까지 다들고 와야한다.
